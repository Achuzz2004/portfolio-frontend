...
export default function Experience({ experience }) {
  if (!experience || experience.length === 0) {
    return (
      ...
      >
        [ DATA_STREAM_EMPTY: NO_RECORDS_FOUND ]
      </motion.div>
    );
  }

  return (
    <section id="experience" className="relative w-full py-16 lg:py-24 bg-transparent overflow-hidden">
      {/* Subtle background gradient that moves on scroll */}
      <motion.div 
        initial={{ opacity: 0 }}
        whileInView={{ opacity: 0.5 }}
        transition={{ duration: 1.5 }}
        className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,rgba(220,38,38,0.03),transparent_50%)] pointer-events-none"
      />
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-8">
        ...
        >
          ...
          <h2 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black text-white italic tracking-tighter uppercase leading-[1.1]">
            Career <span className="text-red-600 relative">
              Trajectory
              <motion.span 
                initial={{ scaleX: 0 }}
              ...
              >
                ...
                <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4 lg:gap-8">
                  ...
                  <div className="flex-1">
                    ...
                    {/* Role with animated underline on hover */}
        ...
        {/* Floating index counter - visible on large screens */}
        <motion.div 
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 0.3 }}
          transition={{ delay: 1 }}
